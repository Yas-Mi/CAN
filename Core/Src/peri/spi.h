...
extern osStatus spi_init(void);
extern osStatus spi_open(SPI_CH ch, SPI_PAR *p_par);
extern osStatus spi_send_recv(SPI_CH ch, uint8_t *snd_data, uint8_t *rcv_data, uint32_t snd_sz);
extern osStatus spi_close(SPI_CH ch);
extern osStatus spi_nss_on(SPI_CH ch, uint32_t dly_time);
extern osStatus spi_nss_off(SPI_CH ch, uint32_t dly_time);
